...
*TODO*: Implementation status definitions +
*TODO*: Overview of all endpoints
...
====== Update screencapture interval
* *Path*: `/exams/{id}/telemetry/{interval-seconds}`
* *Method*: `POST`
* *Description*:
* *Request format*: `Path param`
* *Request structure*: `/exams/{id}/telementry/{interval-seconds}` where `{id}` is a valid numeric ID of an exam and `{interval-seconds}` is a valid number of seconds to wait before requesting the next screencapture from student-clients
* *Response format*: N/A
* *Response structure*: N/A
* *Additional Notes*: N/A
