...
----
{
  "type": "CAPTURE_SCREEN",
  "payload": {
    "frame_type": "ALPHA" | "BETA" | "UNSPECIFIED"
  }
}
----
...
* *Additional Notes*: If `frame_type` is set to unspecified, it is up to the student-client (openbox) to choose which frame is appropriate for the current situation. If a specific frame type is specified the client MUST follow this command.
